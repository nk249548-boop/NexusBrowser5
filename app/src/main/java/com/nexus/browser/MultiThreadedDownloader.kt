...
class MultiThreadedDownloader(private val context: Context) {
    ...
    suspend fun downloadFile(
        ...
        try {
            ...
            val tempFiles = chunks.mapIndexed { i, _ -> File(tempDir, "nexus_chunk_${i}_${System.nanoTime()}.tmp") }
            ...
            try {
                ...
                // FIX — orphaned chunk files on cancel/failure:
                // Previously tempFiles were only deleted inside the merge loop,
                // which never ran if the job was cancelled (or a chunk failed)
                // while downloads were still in flight. That left "nexus_chunk_*.tmp"
                // partial files sitting in cacheDir forever. This finally block
                // guarantees every chunk file is cleaned up on every exit path —
                // success, cancellation, or exception — not just the happy path.
                tempFiles.forEach { f -> if (f.exists()) f.delete() }
            }

        } catch (e: CancellationException) {
            Log.w(TAG, "⚠️ Download cancelled")
            // Ensure the (possibly partial) merged output file doesn't linger either.
    ...
    suspend fun downloadM3u8Stream(
        ...
        try {
            ...
            // FIX: clean up the partial .ts file here too, so this function is
            // safe to call from anywhere — not just MainActivity, which had its
            // own (now-redundant-but-harmless) cleanup at the call site.
            File(outputPath).takeIf { it.exists() }?.delete()
