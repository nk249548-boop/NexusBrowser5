...
class VideoSnifferWebViewClient(
    ...
    private val onImageDetected     : (imageUrl: String) -> Unit = {}
) : WebViewClient() {
    ...
    override fun onPageStarted(view: WebView?, url: String?, favicon: Bitmap?) {
        ...
        // Feature 8: Direct image page detection
        if (url != null && ImageDownloadHelper.isImageUrl(url)) {
            view?.post { onImageDetected(url) }
        }
    ...
    // ── 5. JavaScript: Image Scraper ──────────────────────────────────────────

    private fun injectImageScraperJs(view: WebView?) {
        val js = """
            (function() {
                if (typeof window.__nexusImgScan !== 'undefined') return;
                window.__nexusImgScan = true;

                function reportImg(src) {
                    if (!src || !src.startsWith('http')) return;
                    var ext = src.split('?')[0].toLowerCase();
                    if (ext.match(/\.(jpg|jpeg|png|webp|gif|bmp|svg)$/)) {
                        NexusVideoScraper.onImageFound(src);
                    }
                }
                ...
                if (window.MutationObserver) {
                    new MutationObserver(function(muts) {
                        muts.forEach(function(m) {
                            m.addedNodes.forEach(function(n) {
                                if (n.nodeName === 'IMG') reportImg(n.src);
                                else if (n.querySelectorAll) {
                                    n.querySelectorAll('img[src]').forEach(function(i) {
                                        reportImg(i.src);
                                    });
                                }
                            });
